...
def update_screen(player, camera, mapa):
    camera.x = max(0, min(player.rect.x - SCREEN_WIDTH // 2, (len(mapa.tiles[0]) * TILES_SIZE) - SCREEN_WIDTH))
    camera.y = max(0, min(player.rect.y - SCREEN_HEIGHT // 2, (len(mapa.tiles) * TILES_SIZE) - SCREEN_HEIGHT))
